...
public class GoodsListUpdateAdapter  extends RecyclerView.Adapter<GoodsListUpdateAdapter.MyViewHolder> implements View.OnClickListener  {
    private OnItemClickListener mOnItemClickListener = null;
    private List<GoodsListEntity.DataBean> dataBeans;
    private Context mContext;

    public GoodsListUpdateAdapter(Context context, List<GoodsListEntity.DataBean> list) {
        mContext = context;
        this.dataBeans = list;
    }
    @Override
    public GoodsListUpdateAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_goodslist, parent, false);
        //将创建的View注册点击事件
        view.setOnClickListener(this);
        return new GoodsListUpdateAdapter.MyViewHolder(view);
    }


    @Override
    public void onBindViewHolder(GoodsListUpdateAdapter.MyViewHolder holder, int position) {
        ...
        if (dataBeans.get(position).getPro_status()==1){
            holder.Uppershelf.setText("下架");
        }else if (dataBeans.get(position).getPro_status()==3){
            holder.Uppershelf.setText("重新上架");
        }
        holder.Uppershelf.setOnClickListener(v -> {
            if (dataBeans.get(position).getPro_status()==3){
                ApiInterfaceTwo.ApiFactory.createApi().goodsstatus( dataBeans.get(position).getProducts_id(), 1).enqueue(new Callback<GoodsStatusEntity>() {
                    @Override
                ...
                ApiInterfaceTwo.ApiFactory.createApi().goodsstatus( dataBeans.get(position).getProducts_id(), 3).enqueue(new Callback<GoodsStatusEntity>() {
                    @Override
                    public void onResponse(Call<GoodsStatusEntity> call, Response<GoodsStatusEntity> response) {
                        if (response.body().getFlag() == 1) {
                            ToastUtil.show(response.body().getMsg(),200);
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View itemView) {
            ...
            Uppershelf=itemView.findViewById(R.id.Uppershelf);
        }
    }
