...
import com.product.as.merchants.adapter.ShipperEndUpdateAdapter;
...
public class ShipperEndPager extends BasePager implements NoticeObserver.Observer {
    RecyclerView rcyshaped;
    private ShipperEndAdapter shipperEndAdapter;
    ...
    public ShipperEndPager(Context context) {
        super(context);
    }
    ...
    public View initView(LayoutInflater layoutInflater) {
        ...
        orderlists(String.valueOf(SharedPreferencesUtils.getSp(Constants.SID, "")), 2, page, dates.getText().toString());
        ...
        ApiInterfaceTwo.ApiFactory.createApi().orderlists(sid, status, page, day).enqueue(new Callback<OrderListEntity>() {
            ...
            public void onResponse(Call<OrderListEntity> call, Response<OrderListEntity> response) {
                if (response.body().getFlag() == 1) {
                    count.setText("已发货: " + response.body().getCount());
                    if (page == 0) {
                        dataBeanList.clear();
                    }
                    dataBeanList.addAll(response.body().getData());
                    if (shipperEndAdapter == null) {
                        shipperEndAdapter = new ShipperEndAdapter(context, R.layout.item_shapedend, dataBeanList);
                        rcyshaped.setAdapter(shipperEndAdapter);
                    }
                    shipperEndAdapter.notifyDataSetChanged();

                    shipperEndAdapter.setOnItemClickListener(new RecycleAdapter.OnItemClickListener() {
                        @Override
    ...
    public <T> void update(int what, T t) {
        if (what == Constants.ALLREFRESH2) {
            dataBeanList.get(currentPosition).setIs_print_ticket(1);
            shipperEndAdapter.notifyItemChanged(currentPosition);
        }
    }
