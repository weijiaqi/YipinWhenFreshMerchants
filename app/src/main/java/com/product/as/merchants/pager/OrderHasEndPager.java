...
import com.product.as.merchants.adapter.OrderHasUpdateAdapter;

import com.product.as.merchants.adapter.ShipperEndAdapter;
...
//已完结
public class OrderHasEndPager extends BasePager implements NoticeObserver.Observer {
    private RecyclerView rcyorderhas;
    private ShipperEndAdapter shippedAdapter;
    ...
    public OrderHasEndPager(Context context) {
        super(context);
    }
    ...
    public View initView(LayoutInflater layoutInflater) {
        ...
        View view = layoutInflater.inflate(R.layout.orderhasend_pager, null);
        rcyorderhas = view.findViewById(R.id.rcyorderhas);
        refreshLayout = view.findViewById(R.id.refreshLayout);
        count = view.findViewById(R.id.count);
        dates = view.findViewById(R.id.date);
        rldate = view.findViewById(R.id.rldate);
        dates.setText(TimeUtils.getYesterday());
        rcyorderhas.setLayoutManager(new LinearLayoutManager(context));
        rcyorderhas.addItemDecoration(new MyDecoration(context, MyDecoration.VERTICAL_LIST));
        rldate.setOnClickListener(v -> {
            pvTime.show();
        });
        initTimePicker();
        orderlists(String.valueOf(SharedPreferencesUtils.getSp(Constants.SID, "")), 3, page, dates.getText().toString());

        rcyorderhas.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
        ...
        refreshLayout.setOnLoadMoreListener(refreshLayout -> {
            ...
            if (dataBeanList.size() >= 0) {
                rcyorderhas.smoothScrollToPosition(dataBeanList.size());
            }
        ...
        ApiInterfaceTwo.ApiFactory.createApi().orderlists(sid, status, page, day).enqueue(new Callback<OrderListEntity>() {
            ...
            public void onResponse(Call<OrderListEntity> call, Response<OrderListEntity> response) {
                if (response.body().getFlag() == 1) {
                    count.setText("已完结: " + response.body().getCount());
                    if (page == 0) {
                        dataBeanList.clear();
                    }
                    dataBeanList.addAll(response.body().getData());
                    if (shippedAdapter == null) {
                        shippedAdapter = new ShipperEndAdapter(context,R.layout.item_hasend, dataBeanList);
                        rcyorderhas.setAdapter(shippedAdapter);
                    }
