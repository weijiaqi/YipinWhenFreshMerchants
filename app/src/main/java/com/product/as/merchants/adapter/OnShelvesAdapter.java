...
public class OnShelvesAdapter extends RecycleAdapter<GoodsListEntity.DataBean> {
    public OnShelvesAdapter(Context context, int layoutId, List<GoodsListEntity.DataBean> datas) {
    ...
    public void convert(RecyclerViewHolder holder, GoodsListEntity.DataBean dataBean) {
        ...
        TextView Uppershelf = holder.getConvertView().findViewById(R.id.Uppershelf);
        Uppershelf.setOnClickListener(v -> {
            ApiInterfaceTwo.ApiFactory.createApi().goodsstatus(dataBean.getProducts_id(), 3).enqueue(new Callback<GoodsStatusEntity>() {
                @Override
